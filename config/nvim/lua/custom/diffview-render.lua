-- Patched diffview render: status za nazwą pliku/folderu, kompaktowe wcięcia
...
local function render_file_tree_recurse(depth, comp)
  ...
  -- Status za nazwą (kompaktowy)
  local status_text = get_dir_status_text(ctx, conf.file_panel.tree_options)
  if status_text ~= " " then
    dir:add_text(" " .. status_text, hl.get_git_hl(ctx.status))
  end
